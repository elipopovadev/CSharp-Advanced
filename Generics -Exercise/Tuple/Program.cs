...
namespace Tuple
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var tupleForNamesAndAddres= new MyTupil<string, string>($"{firstName} {lastName}", address);
            ...
            var tupleForNamesAndAmountOfBeer = new MyTupil<string, int>(name, amountOfBeer);

            string[] numbers = Console.ReadLine().Split();
            int firstNumber = int.Parse(numbers[0]);
            double secondNumber = double.Parse(numbers[1]);
            var tupleForNumbers = new MyTupil<int, double>(firstNumber, secondNumber);
